!function (i) {
  ...
  o = function () {
    i('.slides-container .slide-item').addClass('sliderFix'),
    setTimeout(function () {
  ...
  r = function () {
    ...
    i(window).on('load resize', function () {
      ...
      if (matchMedia('only screen and (max-width: 1024px)').matches && (t = 'mobile'), t !== e) if (e = t, 'mobile' === t) {
        ...
        i('#header').find('.header-wrap').after(a),
        n.children('ul').hide(),
        n.children('a').after('<span class="btn-submenu"></span>'),
        i('.btn-menu').removeClass('active')
      } else {
        var o = i('#mainnav-mobi').attr('id', 'mainnav').removeAttr('style');
        o.find('.submenu').removeAttr('style'),
        i('#header').find('.col-md-10').append(o),
        i('.btn-submenu').remove()
      }
    }),
    i('.btn-menu').on('click', function () {
      i('#mainnav-mobi').slideToggle(300),
      i(this).toggleClass('active')
    }),
    i(document).on('click', '#mainnav-mobi li .btn-submenu', function (e) {
      i(this).toggleClass('active').next('ul').slideToggle(300),
  ...
  l = function () {
    ...
    i('.panel-grid .panel-widget-style').each(function () {
      ...
      e && i(this).find('.widget-title').css('color', e),
      t && i(this).find('h1,h2,h3:not(.widget-title),h4,h5,h6,h3 a').css('color', t)
    })
  },
  d = function () {
    e = a.any(),
    null == e && i('.panel-row-style, .slide-item').parallax('50%', 0.3)
  },
  c = function () {
    t = n.any(),
    null != t && i('.slides-container .slide-item').css('background-attachment', 'scroll')
  },
  u = function () {
  ...
  m = function () {
    i('.progress-bar').on('on-appear', function () {
      i(this).each(function () {
        ...
        i(this).parent('.roll-progress').find('.perc').addClass('show').animate({
          width: e + '%'
        }, 3000)
      })
    })
  ...
  p = function () {
    ...
    i(window).on('load scroll', function () {
      ...
      t >= e ? (i('.site-header').addClass('fixed'), i('body').addClass('siteScrolled'))  : (i('.site-header').removeClass('fixed'), i('body').removeClass('siteScrolled')),
      t >= 107 ? i('.site-header').addClass('float-header')  : i('.site-header').removeClass('float-header')
    })
  },
  v = function () {
  ...
  y = function () {
    ...
    i('.toggle-popup').on('click', function (e) {
      e.preventDefault(),
      i(this).siblings().addClass('popup-show');
  ...
  C = function () {
    ...
    i('.project-item').children('.item-wrap').addClass('orches-animation'),
    i('.project-wrap').waypoint(function (t) {
  ...
  T = function () {
    e = a.iOS(),
    i(window).on('load', function () {
      i('#wp-custom-header').fitVids(),
      i('.fluid-width-video-wrapper + #wp-custom-header-video-button').find('i').removeClass('fa-play').addClass('fa-pause'),
      i('.fluid-width-video-wrapper + #wp-custom-header-video-button').on('click', function () {
  ...
  S = function () {
    i('.project-wrap').length && i('.project-wrap').each(function () {
      ...
      t.click(function () {
        ...
        e.find('.isotope-container').isotope({
          filter: a,
          animationOptions: {
            duration: 750,
            easing: 'liniar',
